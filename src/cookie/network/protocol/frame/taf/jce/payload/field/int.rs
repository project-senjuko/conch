...
impl FieldBuild<i32> for Field<i32> {
    fn new() -> Field<i32> { Field { key: ZERO_HEAD, value: 0i32 } }

    fn with_tag(tag: u8) -> Field<i32> { Field { key: HeadData { r#type: INT, tag }, value: 0i32 } }

    fn with_tv(tag: u8, value: i32) -> Field<i32> { Field { key: HeadData { r#type: INT, tag }, value } }

    fn from_bytes(tag: u8, b: &mut Bytes) -> Field<i32> {
        let mut a: Field<i32> = Field::with_tag(tag);
        a.parse(b);
        a
    }
}
...
mod tests {
    ...
    #[test]
    fn to_bytes() { assert_eq!(Field::with_tv(0, 114514_i32).format().to_vec(), vec![2, 0, 1, 191, 82]); }

    #[test]
    fn from_bytes() {
        let a: Field<i32> = Field::from_bytes(0, &mut Bytes::from(vec![0, 1, 191, 82]));
