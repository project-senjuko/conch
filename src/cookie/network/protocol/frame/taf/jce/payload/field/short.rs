...
impl FieldBuild<i16> for Field<i16> {
    fn new() -> Field<i16> { Field { key: ZERO_HEAD, value: 0i16 } }

    fn with_tag(tag: u8) -> Field<i16> { Field { key: HeadData { r#type: SHORT, tag }, value: 0i16 } }

    fn with_tv(tag: u8, value: i16) -> Field<i16> { Field { key: HeadData { r#type: SHORT, tag }, value } }

    fn from_bytes(tag: u8, b: &mut Bytes) -> Field<i16> {
        let mut a: Field<i16> = Field::with_tag(tag);
        a.parse(b);
        a
    }
}
...
mod tests {
    ...
    #[test]
    fn to_bytes() { assert_eq!(Field::with_tv(0, 1919_i16).format().to_vec(), vec![1, 7, 127]); }

    #[test]
    fn from_bytes() {
        let a: Field<i16> = Field::from_bytes(0, &mut Bytes::from(vec![7, 127]));
