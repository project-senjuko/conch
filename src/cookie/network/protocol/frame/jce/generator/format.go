...
func format(j *JceSpec) (b strings.Builder) {
	...
	b.WriteString(HEAD)
	b.WriteString(formatImport(j))
	b.WriteString(IMPORTEND)
...
func formatImport(j *JceSpec) string {
	var b strings.Builder
	var c = map[string]bool{"JceStruct": true}
	for _, f := range j.Spec.Field {
		c[f.Type] = true
	}
	for _, k := range []string{"JBool", "JByte", "JceStruct", "JDouble", "JFloat", "JInt", "JList", "JLong", "JMap", "JShort", "JSList", "JString"} {
		if c[k] {
			b.WriteString(k)
			b.WriteString(SEP1)
		}
	}
	return strings.TrimSuffix(b.String(), SEP1)
}
